...
3.05. 从 `2026-06-24` 起，Claude translator 还会处理 **unsolicited / agent-initiated turn**：当一个后台任务完成并向 Claude 注入 `<task-notification>`（或其它无 daemon `prompt.send` 触发的自发输出）时，`stream_event message_start` 会在没有任何 pending/active turn 的情况下到达。translator 现在在这一刻（且仅在 session id 已知时）合成一个 `Initiator=unknown` 的 turn，使整轮 lifecycle（turn.started / item / turn.completed）正常下发，而不是被 `activeTurn==nil` 全部丢弃。该 turn 没有 `CommandID` 关联的 remote binding，daemon 侧 `normalizeTurnInitiator` 保持其 unknown，`turnSurface` 通过 `threadClaim` 回退把产物路由到当前 own 该 thread 的 surface；若当前没有 surface claim 该 thread，则按既有 thread-replay 语义暂存，等重新 attach 时回放。合成只在 live `message_start` 触发，因此 `--resume` 回放与 bootstrap 帧不会被误判成自发 turn。
...
2. clear 语义，例如 `/stop`、`/detach`、`/mode`、`/new`、`/clear`、`R5` 下的 `/use` / `vscode /follow`：
...
当前 `/new` 已实现为 clear-and-prepare；纯文本 `/clear` 是同一条 `ActionNewThread` alias，不会作为普通 prompt 透传给 backend：
...
其中 `/new` / `/clear` / 菜单 `new` 都直接映射到 `ActionNewThread`；`/history` / 菜单 `history` 都映射到 `ActionShowHistory`。
...
`/new` 已经是当前实现的一部分；`/clear` 是同一产品语义的文本 alias。
