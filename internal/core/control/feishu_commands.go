...
var feishuCommandSpecs = []feishuCommandSpec{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentKind:     FeishuCommandArgumentChoice,
			ArgumentFormHint: "auto",
			ArgumentFormNote: "输入 auto / full / confirm / clear。",
			ArgumentSubmit:   "应用",
			Description:      "查看当前执行权限；bare `/access` 会返回可选参数卡片。",
			Examples:         []string{"/access auto", "/access confirm", "/access clear"},
			Options: []FeishuCommandOption{
				commandOption("/access", "access", "auto", "auto", "把后续飞书消息切到 Claude auto mode，直到 clear 或接管清理。"),
		...
		menuExact: []feishuCommandMatch{
			...
			{alias: "accessauto", action: Action{Kind: ActionAccessCommand, Text: "/access auto"}},
			{alias: "approvalauto", action: Action{Kind: ActionAccessCommand, Text: "/access auto"}},
