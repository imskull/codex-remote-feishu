...
func TestBareModelCommandBuildsDropdownAndManualFormCard(t *testing.T) {
	...
	if len(options) != 8 || options[0].Value != "gpt-5.6-sol medium" || options[1].Value != "gpt-5.6-terra medium" || options[2].Value != "gpt-5.5" || options[3].Value != "gpt-5.4" || options[4].Value != "gpt-5.4-mini" || options[5].Value != "gpt-5.3-codex" || options[6].Value != "gpt-5.2" || options[7].Value != "gpt-5.2-codex" {
